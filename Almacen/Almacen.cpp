...
int main()
{
    ...
    */
     ...
     * agregamos PASANDO EL APUNTADOR A ESE ARTÍCULO y agregándolo a los artículos
    ...
    */
    for (int i = 0; i < numArticulos; i++)
    {
        ...
        int resp=0;
        string art;
        string* artPtr=nullptr;
        
        misArticulos[i]=CrearArticulo(i);
        artPtr=&misArticulos[i];
        
        for (int j=0;j<2;j++){
            
            cout<<"¿Desea agregar el articulo a la bodega"<< j<<"?(1 si, 0 no)"<<endl;
            
            cin>>resp;
            if (resp==1){
                do{
                    
                exe=misBodegas[j].AgregarArticulo(artPtr);
                
                } while(exe==true);
                AgregarArticuloABodega(artPtr,misBodegas[j]);
